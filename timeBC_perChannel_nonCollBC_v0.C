...
void timeBC_perChannel_nonCollBC_v0(const char * file="chrg_QhistoNonInterBC_543641", const char* folder="."){
  ...
  for (int i = 0; i < nChannels; i++) {
    htBC_A[i] = (TH2F*)gDirectory -> Get(TString::Format("BCvsCharge_notCollidingBC_%d_H",i));
    //htBC_C[i] = (TH2F*)gDirectory -> Get(TString::Format("timePMTWithBC_Conly_%d_H",i));
 ...
 // Text Prints for the canvas
  ...
  TCanvas* cCT_Aonly = new TCanvas("cCT_Aonly", "BC A-side ONLY, FDD-C time", 1600,1000);
  cCT_Aonly->Divide(2,4);
  TCanvas* cAT_Aonly = new TCanvas("cAT_Aonly", "BC A-side ONLY, FDD-A time", 1600,1000);
  ...
  for (int i = 0; i < 16; i++) {
    ...
    if (i<8) {
      cCT_Aonly->cd(i+1);
       if (setLog) cCT_Aonly->cd(i+1)->SetLogz();
    }
    else{
        cAT_Aonly->cd(i-7);
          if (setLog) cAT_Aonly->cd(i-7)->SetLogz();
    }
  ...
  for (int i = 0; i < 16; i++) {
    ...
    if (i<8) {
      cCT_Conly->cd(i+1);
       if (setLog) cCT_Conly->cd(i+1)->SetLogz();
    }
    else{
        cAT_Conly->cd(i-7);
          if (setLog) cAT_Conly->cd(i-7)->SetLogz();
    }

    htBC_C[i]->GetYaxis() -> SetRangeUser(rangeMinY,rangeMaxY);
    htBC_C[i]->Draw("COLZ");
 ...
 for (int i = 0; i < 16; i++) {
    ...
    if (i<8) {
      cCT_TimeCharge->cd(i+1);
       if (setLog) cCT_TimeCharge->cd(i+1)->SetLogz();
    }
    else{
        cAT_TimeCharge->cd(i-7);
          if (setLog) cAT_TimeCharge->cd(i-7)->SetLogz();
    }
