...
TH2F *BCvsCharge_notCollidingBC[16];

//-----------------------------------------------
// Flags
TH1F *flagsPMT_H[16];
TString feeNames[4] = {"isGateAndTVDC","isGate","isInTVDC","QhistoNonInterBC"};
...
void initHistograms()
{
 ...
 mHistChDataBits = new TH2F("ChannelDataBits", "ChannelData bits per ChannelID;Channel;Bit", sNCHANNELS_PM, 0, sNCHANNELS_PM, mMapChTrgNames.size(), 0, mMapChTrgNames.size());
  ...
  for(int i=0;i<16;i++) {
    ...
    BCVsChAonlyBC[i]= new TH2F(Form("BCVsChAonlyBC%d_H",i),"BC Vs Charge (A only) ; BC;charge",3565,-0.5,3564.5, 4200,-5.5,4094.5);
    ChargeVsTime[i] = new TH2F(Form("ChargeVsTime%d_H",i),"Charge Vs time ;charge (ADC counts);time (ns)",4100,-10.5,4189.5, 600,-30.,30.);
// BC distribution of signals outside a time window
    BCvsCharge_notCollidingBC[i]= new TH2F(Form("BCvsCharge_notCollidingBC_%d_H",i),"BC Vs Charge of non-colliding bunches; BC;charge",3565,-0.5,3564.5, 4200,-5.5,4094.5);
...
void saveHistograms(TString useBit,int runNumber, double ChargeCoincThreshold = 0)
{
  ...
  for(int i=0;i<16;i++) {
    ...
    BCvsCharge_notCollidingBC[i]->Write();
  }
