...
void anaDigits_charges(int useFeeBits= 3, unsigned int runNumber = 543389, double adcCoincThreshold= 15, bool processNTrees=1, int iFile=0 )
{
  ...
  for (int iTree = 0; iTree<nTree; iTree++) {
    ...
    for(int iBC=0;iBC<nBC;iBC++) {
      ...
      for(int iPMT = 0; iPMT<nPMT;iPMT++) {
        ...
        // fill only non interacting bunches A and C
        if (Aside.test((int)bc)) {
          BCVsChAonlyBC[iChannel]->Fill(bc,adc[iChannel]);
        }
        if (Cside.test((int)bc)) {
          BCVsChConlyBC[iChannel]->Fill(bc,adc[iChannel]);
        }
        ...
        if(CollidingBunch.test((int)bc)){
          BCvsCharge_CollidingBC[iChannel]->Fill(bc,adc[iChannel]);
        }

        // Fill only after these interacting bunches
        if (bc == 1292 || bc == 1293 || bc == 1294|| bc == 1295|| bc == 1296 )
        {
          nextBCvsCharge[iChannel]->Fill(bc,adc[iChannel]);
        }
